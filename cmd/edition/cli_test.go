...
func TestFindConfigPathSupportsLongAndShortForms(t *testing.T) {
	tests := []struct {
		...
		{args: []string{"--config", "first.yaml", "create", "--config=last.yaml"}, want: "last.yaml"},
		{args: []string{"-c", "first.yaml", "create", "-clast.yaml"}, want: "last.yaml"},
		{args: []string{"-c=selected.yaml", "create"}, want: "selected.yaml"},
	}
	for _, test := range tests {
		if got := findConfigPath(test.args); got != test.want {
			t.Fatalf("findConfigPath(%#v) = %q, want %q", test.args, got, test.want)
		}
	}
}
...
func TestLoadCLIConfigAppliesEnvironmentAfterFile(t *testing.T) {
	configPath := filepath.Join(t.TempDir(), "config.yaml")
	if err := os.WriteFile(configPath, []byte("hardcover:\n  token: from-file\nsync:\n  state_file: file-state.json\n"), 0600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("HARDCOVER_TOKEN", "from-env")
	t.Setenv("SYNC_STATE_FILE", "env-state.json")
	cfg, err := loadCLIConfig(configPath)
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Hardcover.Token != "from-env" || cfg.Sync.StateFile != "env-state.json" {
		t.Fatalf("environment did not override config: token=%q state=%q", cfg.Hardcover.Token, cfg.Sync.StateFile)
	}
}

func TestValidateCLIABSConfigRejectsUnsupportedTrustAndURLPolicy(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Audiobookshelf.NetworkTrust = "unsupported"
	if err := validateCLIABSConfig(cfg); err == nil {
		t.Fatal("unsupported trust should be rejected")
	}

	cfg.Audiobookshelf.NetworkTrust = audiobookshelf.NetworkTrustPublicOnly
	cfg.Audiobookshelf.URL = "http://abs.example"
	if err := validateCLIABSConfig(cfg); err == nil {
		t.Fatal("public_only should reject HTTP")
	}

	cfg = config.DefaultConfig()
	if err := validateCLIABSConfig(cfg); err != nil {
		t.Fatalf("Hardcover-only CLI config should remain valid without ABS settings: %v", err)
	}
}
