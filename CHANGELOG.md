...
- **Explicit edition creation and Audiobookshelf URL trust**: Added profile-authorized `POST /api/profiles/{id}/edition-drafts/create` and migrated `edition create` to regional audiobook imports and format-aware ebook insertion. Requests use a completed, non-dry-run needs-review run record; verified results are saved as local matches. Superseded candidates and items with saved associations are rejected before remote writes. Audiobook responses include a metadata preview whose release date uses matching Audnex data when available, otherwise the Audiobookshelf date. The CLI requires an explicit `--state-file` when associating an ABS item, including when `abs_item_id` comes from input JSON. Added deployment-wide `audiobookshelf.network_trust` / `AUDIOBOOKSHELF_NETWORK_TRUST` (`allow_private` by default or `public_only`), enforced for Audiobookshelf connections, and `Retry-After: 1` on busy draft/create responses. After a remote success and local-save failure, operators are told to verify the Hardcover result before retrying because the retry may create another edition. Failures during or after a Hardcover attempt carry the same caution when the remote result cannot be verified.
