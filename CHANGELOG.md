...
- **Create a Hardcover edition for a `needs_review` book (API)**: When a sync run marks a book `needs_review` and its record has a Hardcover candidate, an authenticated caller with write permission on the profile can fetch a draft of a new Hardcover edition built from the Audiobookshelf item (`GET /api/profiles/{id}/runs/{runID}/books/{bookID}/edition-draft`) and then create it (`POST /api/profiles/{id}/runs/{runID}/books/{bookID}/edition`). The Hardcover book comes from the run record and the cover from Audiobookshelf, unknown request fields are rejected, an existing audiobook edition of the same Hardcover book with the same ASIN, ISBN-13 or ISBN-10 (also under the converted ISBN form) is found before creating and returned unchanged instead of duplicated (one on another book, or whose book cannot be confirmed, is rejected with `409`), a book whose Audiobookshelf item has no ASIN or valid ISBN cannot get an edition (`409`, since a sync could never match it), a create request needs an ASIN or ISBN (ISBNs may contain hyphens), and an ebook-only Audiobookshelf item (an ebook file and no audio) gets an ebook edition (Hardcover's ebook reading format, no narrators or audio length), with existing-edition detection limited to ebook editions, and a dry-run profile creates nothing. If the edition is created but its cover cannot be uploaded, the response still succeeds and reports a `warnings` entry. The Audiobookshelf token is used only on the server and is never returned. There is no web UI for this yet. By @Snuffy2.
...
- **Hyphenated ISBNs are kept**: An Audiobookshelf ISBN written with hyphens or spaces was dropped from the mismatch export and the edition draft, because ISBN-10 and ISBN-13 were told apart by exact string length. It is now normalized, and the edition draft also fills the other ISBN form when it can be derived. By @Snuffy2.
...
- **Ebook items export as ebook editions**: The mismatch JSON export (`edition_*.json` files consumed by the `edition` CLI) now marks an ebook item with `"reading_format": "ebook"` and an `Ebook` edition format, and leaves out the audiobook-only `Unabridged` default, audio length and narrators. The `edition` command accepts an optional `reading_format` (`audiobook` by default, or `ebook`); `ebook` creates the edition with Hardcover's ebook reading format and looks for an existing edition among ebook editions only. Audiobook items export and create exactly as before. By @Snuffy2.
