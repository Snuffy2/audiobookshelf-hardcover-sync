...
- **Persistent, read-only edition matching**: Sync saves verified regional Audible matches in state schema `4.0` and reuses them across runs while still reading existing `3.0` checkpoints; other ASIN, ISBN, and ebook matches remain read-only lookups. A profile API forgets a saved match and its checkpoint. When user-book creation, edition correction, or a read mutation fails for a saved match, sync confirms edition absence with an uncached lookup before forgetting that match; transient lookup failures preserve it for retry, and a confirmed missing edition is reconsidered on a later sync. Failed edition corrections stop before progress is applied to another edition. Dry runs leave saved matches and checkpoints in place, and sync makes no Hardcover catalogue writes. Cross-process locking protects the resolved state file through each transaction, and profile configuration changes are serialized with forget operations. By @Snuffy2. (#201)
...
- **Explicit edition creation and Audiobookshelf URL trust**: Added profile-authorized `POST /api/profiles/{id}/edition-drafts/create` and migrated `edition create` to regional audiobook imports and format-aware ebook insertion. Requests use a completed, non-dry-run needs-review run record; verified results are saved as local matches. Audiobook responses include a metadata preview whose release date uses matching Audnex data when available, otherwise the Audiobookshelf date. The CLI accepts an ABS item ID and state-file override. Added deployment-wide `audiobookshelf.network_trust` / `AUDIOBOOKSHELF_NETWORK_TRUST` (`allow_private` by default or `public_only`), enforced for Audiobookshelf connections, and `Retry-After: 1` on busy draft/create responses. A remote success followed by local-save failure can be retried with the same create request to save the match.
