...
- **Explicit edition creation API**: Added profile-authorized `POST /api/profiles/{id}/edition-drafts/create`. Requests use a completed, non-dry-run needs-review run record; audiobooks use a regional Audible import and ebooks use format-aware `insert_edition`, and verified results are saved as local matches. Superseded candidates and items with saved associations are rejected before remote writes. Audiobook responses include a metadata preview whose release date uses matching Audnex data when available, otherwise the Audiobookshelf date. Busy draft/create responses include `Retry-After: 1`. After a remote success and local-save failure, operators are told to verify the Hardcover result before retrying because the retry may create another edition. Failures during or after a Hardcover attempt carry the same caution when the remote result cannot be verified.
- **Audiobookshelf URL trust**: Added deployment-wide `audiobookshelf.network_trust` / `AUDIOBOOKSHELF_NETWORK_TRUST` (`allow_private` by default or `public_only`), enforced at the shared Audiobookshelf client for profile configuration, sync, edition drafts, image cover fetches, and the `edition` CLI, including redirect targets. The `edition` CLI now applies environment overrides such as `AUDIOBOOKSHELF_NETWORK_TRUST` and `HARDCOVER_TOKEN`, no longer requires `config.yaml`, and keeps its standard output free of configuration summaries. Invalid profile URLs are rejected with HTTP 400, and draft `429` responses now include `Retry-After: 1`. By @Snuffy2.
...
- **Reject stale edition sources after title or author changes**: Edition creation now verifies the Audiobookshelf title and author from the selected sync run against the current item and any newer needs-review candidate before contacting Hardcover or saving an association.
