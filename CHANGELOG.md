...
- **Create a Hardcover edition for a `needs_review` book (API)**: When a sync run marks a book `needs_review` and its record has a Hardcover candidate, an authenticated caller with write permission on the profile can fetch a draft of a new Hardcover edition built from the Audiobookshelf item (`GET /api/profiles/{id}/runs/{runID}/books/{bookID}/edition-draft`) and then create it (`POST /api/profiles/{id}/runs/{runID}/books/{bookID}/edition`). The Hardcover book comes from the run record and the cover from Audiobookshelf, unknown request fields are rejected, an existing edition with the same ASIN is returned instead of duplicated, and a dry-run profile creates nothing. If the edition is created but its cover cannot be uploaded, the response still succeeds and reports a `warnings` entry. The Audiobookshelf token is used only on the server and is never returned. There is no web UI for this yet. By @Snuffy2.
