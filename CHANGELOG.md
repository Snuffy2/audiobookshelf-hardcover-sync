...
- **Create a Hardcover edition for a `needs_review` book (API)**: When a sync run marks a book `needs_review` and its record has a Hardcover candidate, an authenticated caller with write permission on the profile can fetch a draft of a new Hardcover edition built from the Audiobookshelf item (`GET /api/profiles/{id}/runs/{runID}/books/{bookID}/edition-draft`) and then create it (`POST /api/profiles/{id}/runs/{runID}/books/{bookID}/edition`). The Hardcover book comes from the run record and the cover from Audiobookshelf, unknown request fields are rejected, an existing edition with the same ASIN or ISBN-13 on the same Hardcover book is returned unchanged instead of duplicated (one on a different book is rejected with `409`), and a dry-run profile creates nothing. If the edition is created but its cover cannot be uploaded, the response still succeeds and reports a `warnings` entry. The Audiobookshelf token is used only on the server and is never returned. There is no web UI for this yet. By @Snuffy2.
...
- **Edition creation no longer adopts another book's edition**: The `edition` command and the shared edition creator now refuse to adopt an existing edition, found by ASIN or ISBN-13, that belongs to a different Hardcover book, and no longer upload or attach a cover to an edition they reuse. By @Snuffy2.
