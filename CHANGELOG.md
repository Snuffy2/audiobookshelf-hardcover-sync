...
- **Audiobookshelf URL trust**: Added deployment-wide `audiobookshelf.network_trust` / `AUDIOBOOKSHELF_NETWORK_TRUST` (`allow_private` by default or `public_only`), enforced at the shared Audiobookshelf client for profile configuration, sync, edition drafts, image cover fetches, and the `edition` CLI, including redirect targets. The `edition` CLI now applies environment overrides such as `AUDIOBOOKSHELF_NETWORK_TRUST` and `HARDCOVER_TOKEN`, no longer requires `config.yaml`, and keeps its standard output free of configuration summaries. Invalid profile URLs are rejected with HTTP 400; drafts with a saved URL disallowed by the current policy return HTTP 409, and draft `429` responses include `Retry-After: 1`. URLs must be absolute, and Audiobookshelf requests no longer use environment proxies; see the migration guide. By @Snuffy2.
