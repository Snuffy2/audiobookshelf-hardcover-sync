...
- **Create editions from needs-review items (API)**: On request, create or reuse an edition through format-aware ebook insertion or regional Audible import, validate the returned book and format, and save the match for the next sync. Stale needs-review records are rejected before any Hardcover write, and dry-run profiles make no external change. By @Snuffy2.
- **Standalone edition CLI migration**: `edition create` now imports audiobooks through the regional Audible resolver and inserts ebooks with format-aware `insert_edition`; `edition.Creator` no longer sends audiobooks to `insert_edition`. The mismatch export adds an optional `abs_item_id`. When an Audiobookshelf item ID is supplied by flag or input JSON, the CLI verifies the item and saves the verified local match in `--state-file` or the configured `sync.state_file` under the state-file lock; a different submitted ASIN is recorded as the user's correction. An explicit `asin_region` is imported without an Audnex lookup. After a remote success and local-save failure, operators are told to verify the Hardcover result before retrying because the retry may create another edition.
