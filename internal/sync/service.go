...
func (s *Service) processBook(ctx context.Context, book models.AudiobookshelfBook, userProgress *models.AudiobookshelfUserProgress) error {
	...
	if findErr != nil {
		...
		if errors.Is(findErr, errHardcoverTitleOnly) || strings.Contains(findErr.Error(), "found by title/author only") {
			...
			const reason = "found by title/author only - manual verification required"
			setOutcome(OutcomeNeedsReview, reason)
			// Publish the attention item before optional Hardcover enrichment. The
			// enrichment path can perform several external requests and must not
			// delay live status visibility.
