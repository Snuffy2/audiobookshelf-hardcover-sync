...
func syncSummaryResponseFromSnapshot(snapshot *sync.SyncSnapshot) types.SyncSummaryResponse {
	if snapshot == nil {
		return types.SyncSummaryResponse{
			BooksNotFound:    make([]types.BookNotFoundInfo, 0),
	...
	copyOf := cloneSnapshotForResponse(snapshot)
	response := types.SyncSummaryResponse{
		Snapshot:            copyOf,
		UserID:              copyOf.UserID,
		RunID:               copyOf.RunID,
		RunStartedAt:        copyOf.RunStartedAt,
		State:               copyOf.State,
		BooksTotal:          copyOf.BooksTotal,
		ProcessedSoFar:      copyOf.ProcessedSoFar,
		ProcessedCount:      copyOf.ProcessedCount,
		OutcomeCounts:       copyOf.OutcomeCounts,
		BookOutcomes:        append([]sync.BookOutcomeRecord(nil), copyOf.BookOutcomes...),
		AttentionRecords:    append([]sync.BookOutcomeRecord(nil), copyOf.AttentionRecords...),
		TotalBooksProcessed: copyOf.TotalBooksProcessed,
		BooksSynced:         copyOf.BooksSynced,
		BooksNotFound:       make([]types.BookNotFoundInfo, 0, len(copyOf.BooksNotFound)),
		Mismatches:          append([]mismatch.BookMismatch(nil), copyOf.Mismatches...),
	}
	for _, book := range copyOf.BooksNotFound {
		response.BooksNotFound = append(response.BooksNotFound, types.BookNotFoundInfo{
			Title:  book.Title,
			Author: book.Author,
		})
	}
