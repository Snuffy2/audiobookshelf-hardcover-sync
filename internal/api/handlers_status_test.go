...
func TestSummaryResponseIncludesOneCurrentRunSnapshot(t *testing.T) {
	started := time.Date(2026, time.September, 12, 22, 0, 0, 0, time.UTC)
	...
	response := syncSummaryResponseFromSnapshot(snapshot)
	if response.Snapshot == snapshot {
		t.Fatal("response snapshot aliases the service snapshot")
	}
	if response.RunID != snapshot.RunID || response.ProcessedSoFar != 3 {
		t.Fatalf("snapshot metadata was not exposed: %#v", response)
	}
	if response.OutcomeCounts != snapshot.OutcomeCounts {
		t.Fatalf("outcome counts were not exposed: %#v", response.OutcomeCounts)
	}
	if len(response.BookOutcomes) != 1 || len(response.AttentionRecords) != 1 {
		t.Fatalf("snapshot details were not exposed: %#v", response)
	}

	response.Snapshot.BookOutcomes[0].Title = "changed"
	response.BookOutcomes[0].Title = "changed again"
	if snapshot.BookOutcomes[0].Title != "Book A" {
		t.Fatal("response book outcomes were not copied")
	}
	...
	for _, key := range []string{"snapshot", "run_id", "run_started_at", "state", "books_total", "processed_so_far", "outcome_counts", "book_outcomes", "attention_records", "total_books_processed", "books_synced", "books_not_found", "mismatches"} {
